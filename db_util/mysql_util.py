...
from db_util.db_config.mysql_config import db_config
from db_util.mysql_pool import get_pool_connection
from util.log_util import get_caller_function, logging, get_caller_info_total
import sys
from util.logger_err import rootLogger
...
"""
def insertOrUpdatePatch(_db, _sql, _args):
    result = 0
    conn = get_pool_connection(_db)
    cursor = conn.cursor(buffered=True)
    #conn.autocommit(True) #replace False -> True
...
# FUNCTION  更新或者删除
# ===============================================
def insertOrUpdate_getId(_db, _sql, _args):
    result = 0
    id = 0
    config = db_config[_db]
    conn = MySQLdb.connect(host=config['host'], port=config['port'], user=config['user'], passwd=config['passwd'],
...
def try_for_threes(_db, _sql, _args):
    caller = get_caller_function()
    for i in xrange(0, 3, 1):
        sleep(2)
        caller(_db, _sql, _args)
